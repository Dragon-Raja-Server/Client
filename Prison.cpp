...
void CPrison::RecvPrisonSystemResult(t_packet *tagpPacket)
{
	...
	case CPrison::PSR_ALREADY_REGISTERED_BAD_USER:
		{
			::AddCurrentStatusMessage(255, 0, 0, lan->OutputMessage(0,634));		// "이미 불량 유저로 등록되어 있습니다."
		}
		break;

	case CPrison::PSR_MOVE_TO_PRISON:
		{
			::AddCurrentStatusMessage(255, 255, 0, lan->OutputMessage(0,636));		// "유저님은 게임 규정에 어긋나는 행동을 하여 감옥으로 이동합니다."
			m_bIsBadUser = true;
		}
		break;
	...
	case CPrison::PSR_MOVE_TO_VILLAGE:
		{
			::AddCurrentStatusMessage(255, 255, 0, lan->OutputMessage(0, 638));		// "감옥에서 풀려나 마을로 이동합니다."
			m_bIsBadUser = false;
		}
		break;
	...
	case CPrison::PSR_NOT_MOVE:
		{
			::AddCurrentStatusMessage(255, 0, 0, lan->OutputMessage(0, 639));		// "감옥에서는 할 수 없는 일입니다"
		}
		break;
		
	default:
		{
			::AddCurrentStatusMessage(255, 0, 0, lan->OutputMessage(0,635));		// "감옥 시스템 처리 에러!!."
		}
		break;
	}
}	//> LTH-040805-KO
