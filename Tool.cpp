	...
	case WM_COMMAND:
		...
		case IDOK:
			t_packet p;
				...
				p.u.ForceNationWar.isStart = true;
				...
				case BTN_IMPRISON:
				{
					...
					if (0 < nCount)
					{
						...
						for (nIndex = 0; nIndex < nCount; ++nIndex)
						{
							...
							sprintf(szTemp, lan->OutputMessage(7, 301), szName);		// "'%s'를 감옥으로 이동합니까?"
							if (::YesOrNo(szTemp, lan->OutputMessage(8, 1)) == IDYES)	// "운영자 모드" 메시지 박스.
							{
				...
				case BTN_RELEASE:
				{
					...
					if (0 < nCount)
					{
						...
						for (nIndex = 0; nIndex < nCount; ++nIndex)
						{
							...
							sprintf(szTemp, lan->OutputMessage(7, 302), szName);		// "'%s'를 출소 시킵니까?"
							if (::YesOrNo(szTemp, lan->OutputMessage(8, 1)) == IDYES)	// "운영자 모드" 메시지 박스.
							{
