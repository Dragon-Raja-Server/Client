...
extern RareMain g_itemref[MAX_ITEM_MUTANT_KIND]; // CSD-030419
...
//void BuffTimer()//reece test
//{
//	MAGICLIST* TempList = NULL;
//	packet* p{0};
//	 //std::vector<NewBuffTime_> bufftimer;
//  
//	/* LPCHARACTER pTarget = ::ReturnCharacterPoint(pPacket->idTarget);
//	 if (pTarget == NULL)  return;*/
//
//	 int duration= p->u.magic.server_magic_result_t.wDuration;
//	Spr* icon=p->u.BuffTime.Icon;
//	// int spellid=p->u.AddEffect.wEffectNo;
//
//	 //check if theirs a active buff.
//	// if (!duration)return;
//	 BuffTime_ bufftimer;
//	 std::vector<BuffTime_>::iterator it;
//
//	 // MAGICLIST* pMagic = ::FindList(g_lpML, pPacket->nMagic, pPacket->idTarget);
//	
//	//duration = bufftimer[1].duration = 100;
//
//	//bufftimer[2].Icon= magic->GetImage();
//
//	//bufftimer.push_back({ bufftimer[0].spellid });
//	bufftimer.push_back({ bufftimer[i].duration });
//	
//	//bufftimer.emplace_back(bufftimer[0].spellid);
//	//bufftimer.emplace_back(bufftimer[1].duration);
//	//bufftimer.emplace_back(bufftimer[2].Icon);
//
//
//
//	uint16_t verticalSpaceCount = 0;
//	for (auto it = g_BuffTime.begin(); it != g_BuffTime.end(); /*do nothing*/)
//	{
//
//
...
//		if (duration < 0)
//		{
//			// buff is gone remove from vector and replace the invalidated iterator
//			it = g_BuffTime.erase(it);
//			continue;
//		}
//
//		// TODO put magic icon here.
//
//		//if (icon == (*it).Icon->img)
//		//{
//		//	bufftimer = (*it);
//		//	break;
//		//}
//		HprintBold((SCREEN_WIDTH / 2) - 40, 40 + verticalSpaceCount, RGB(20, 255, 20), 0, "buff : %d", g_ClientTime + duration);
//
//		// if next buff exists increment y so the icons don't stack on each other.
//		verticalSpaceCount += 10;
//
...
//					else
			...
			else			
			{
				if (ch->nRace != HUMAN)	
				{
					...
					//::HprintBold( x -tttx+ 60, y-40-80-ch->height , RGB( 0, 0xff, 0 ),0, "(Lv:%d)",ch->lv );		// 0808 YGI NPC//reece new
					...
					const int nBaseY = y - ch->height - 19;
						...
						//HprintBold(nBaseX, nBaseY, RGB(0xff, 0xff, 0x00), 0, "Lv%d.%s", ch->lv, g_infNpc[ch->sprno].szName);	// YonMy;5; NPC 레벨 표시
					...
					{	// 기본 속성
						...
						//HprintBold(nBaseX + nOffset, nBaseY, RGB(0xff, 0xff, 0), 0, "%s", g_infNpc[ch->sprno].szName);//lv
						//HprintBold(nBaseX, nBaseY, RGB(0xff, 0xff, 0x00), 0, "Lv%d.%s(%d)", ch->lv, g_infNpc[ch->sprno].szName, ch->id);
						HprintBold(nBaseX, nBaseY, RGB(0xff, 0xff, 0x00), 0, "Lv%d.%s", ch->lv, g_infNpc[ch->GetSprNo()].szName);
					}
				}
				// Tamming된 Character
				if (ch->HostName[0]) 
		...
		if(GetSysInfo( SI_GAME_MAKE_MODE ) )
		{
			::HprintBold( x -tttx-30, y-4 - ch->height + 32 , RGB( 0xff, 0xff, 0xff ),0, "%d, %d ,%d", ch->sprno, ch->id,ch->lv );//reece
		}
#endif
	}	
